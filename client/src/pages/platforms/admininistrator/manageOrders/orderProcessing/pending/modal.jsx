...
const CustomModal = ({ isOpen, setIsOpen, selected, readOnly = false }) => {
  ...
  const computeDiscrepancyAmount = () => {
    const { received = 0 } = form || {};
    const discrepancy = liters?.incoming - received;
    const amount = !readOnly
      ? discrepancy * fuel?.pricing?.cost
      : (liters?.incoming - liters?.received) * fuel?.pricing?.cost;
    return amount > 0 ? `₱${formattedAmount(amount)}` : "₱0.00";
  };

  const computeReceivedAmount = () => {
    const { received = 0 } = form || {};
    const amount = !readOnly
      ? received * fuel?.pricing?.cost
      : liters?.received * fuel?.pricing?.cost;
    return amount > 0 ? `₱${formattedAmount(amount)}` : "₱0.00";
  };
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogContent className="max-w-[50rem]  ">
        <DialogHeader>
          <DialogTitle>
            {!readOnly ? "Incoming Order" : "Received Order"}
          </DialogTitle>
          <DialogDescription>
            {!readOnly
          ...
          <div className="flex flex-col gap-1">
            ...
            <div className="mt-5 grid grid-cols-1 gap-7">
              ...
              <div className="grid grid-cols-2 gap-10">
                <div className="grid w-full max-w-sm items-center gap-1.5">
                  <Label htmlFor="Amount for Received Liters">
                    Amount for Received Liters
                  </Label>
                  <Input
                    type="text"
                    id="Amount for Received Liters"
                    readOnly
                    value={computeReceivedAmount()}
                    placeholder="Enter receive liters here.."
                  />
                  <h2 className=" text-[#808080] italic text-[13px]">
                    Automatically calculated as: Receive Liters * Cost Per Liter
                  </h2>
                </div>
                <div className="grid w-full max-w-sm items-center gap-1.5">
                  <Label htmlFor="Amount for Received Liters">
                    Amount for Discrepancy Liters
                  </Label>
                  <Input
                    type="text"
                    id="Amount for Received Liters"
                    readOnly
                    value={computeDiscrepancyAmount()}
                    placeholder="Enter receive liters here.."
                  />
                  <h2 className=" text-[#808080] italic text-[13px]">
                    Automatically calculated as: Discrepancy Liters * Cost Per
                    Liter
                  </h2>
                </div>
              </div>
              <div className="grid w-full gap-1.5">
                ...
                <Textarea
                  value={
                    !readOnly ? form?.remarks : selected?.received?.remarks
                  }
                  readOnly={readOnly}
                  onChange={({ target }) =>
