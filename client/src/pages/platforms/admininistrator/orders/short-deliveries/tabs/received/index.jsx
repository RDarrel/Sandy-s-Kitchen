...
const ReceivedShortDeliveriesTab = () => {
      ...
      <div className="flex items-center justify-center rounded-2xl border border-dashed border-border bg-muted/20 p-7 text-center sm:p-8">
        <div className="space-y-2">
          <p className="text-sm font-semibold text-foreground">
            No received short deliveries
          </p>
          <p className="text-xs text-muted-foreground">
            Completed short deliveries will show up here once resolved.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-3">
      {rows.map((purchase) => {
        const statusKey = String(purchase?.status || "resolved").toLowerCase();
        const meta = statusMeta[statusKey] || statusMeta.resolved;
...
export default memo(ReceivedShortDeliveriesTab);
