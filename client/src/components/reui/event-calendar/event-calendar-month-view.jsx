...
function restoreChipFocus(
  ...
  if (index < 0) return
  const chip = root.querySelectorAll("[data-slot=event-calendar-event]")[index]
  if (!chip) return
...
function EventCalendarMonthWeek({
  ...
  const gridPos = (colStart, colSpan) => {
    let start = -1
    let end = -1
  ...
  const hiddenBarKeysByCol = week.map((_, col) =>
    new Set(bars
  ...
  const dragGhost = useEventCalendarSelector((state) => {
    ...
    const lastDayMs = zonedStartOfDay(new Date(drag.proposedEnd.getTime() - 1), settings.timeZone).getTime()
    ...
    <div
      ...
      {showWeekNumber && (
        <div
          ...
          {settings.i18n.labels.week(getWeek(toZoned(week[0], settings.timeZone), {
      ...
      {week.map((day, col) => (
        <EventCalendarMonthCell
          ...
          reservedLanes={visibleBars.reduce((max, b) =>
            covers(b, offsets[col]) ? Math.max(max, (b.lane ?? 0) + 1) : max, 0)}
...
function EventCalendarMonthCell({
  ...
  // the spot).
  const inlineDrop = useEventCalendarSelector((state) => {
    const drag = state.drag
  ...
  const dropPlaceholder = inlineDrop ? (
    <div
      ...
      className={cn("shrink-0 rounded-sm border border-dashed", inlineDrop.valid
        ? "border-(--ec-event-color)/50 bg-(--ec-event-color)/8"
        : "border-destructive/70 bg-destructive/10", viewConfig.classNames?.dragGhost)}
      style={
        {
  ...
  const defaultContent = (
    <>
      <div
        ref={contentRef}
        className={cn(// gap-0.5 pairs with the 0.125rem subtraction in the lane spacer
        ...
        "flex min-h-0 flex-1 flex-col gap-0.5 overflow-hidden px-1 pt-1.5", viewConfig.classNames?.monthCellContent)}>
        {reservedLanes > 0 && (
          <div
            aria-hidden
            className="shrink-0"
      ...
      {inDraft && (
        <span
          ...
          {inDraft.isStart && draftRange && (
            <span
              className={cn("absolute inset-x-1 bottom-1", EVENT_CALENDAR_SLOT_DRAFT.label)}>
...
function EventCalendarMoreIndicator({
  ...
  const isDragging = useEventCalendarSelector((state) => state.drag !== null)
  useEffect(() => {
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger
        ...
        className={cn(
          ...
          dropInto &&
            cn("flex shrink-0 items-center border border-dashed", dropInto.valid
        ...
        onClick={(e) => {
          e.stopPropagation()
          const verdict = settings.onMoreClick?.(day, segments.map((segment) => segment.occurrence), e)
          if (verdict === false) {
            e.preventDefault()
            setOpen(false)
          }
        ...
        <div
          ...
          <div className="flex flex-col gap-1 py-1 ps-1 pe-4">
            {segments.map((segment) => (
              <EventCalendarEvent key={segment.occurrence.key} segment={segment} className="py-0.5" />
            ))}
          </div>
        </div>
      ) : (
        <ScrollArea
          className="-me-2 min-h-0 **:data-[slot=scroll-area-viewport]:max-h-(--ec-more-max-height,16rem)">
          <div className="flex flex-col gap-1 py-1 ps-1 pe-4">
            {segments.map((segment) => (
              <EventCalendarEvent key={segment.occurrence.key} segment={segment} className="py-0.5" />
            ))}
