...
function EventCalendarResourceView({
  ...
  const resources = useMemo(() =>
    flattenResources(settings.resources)
      ...
      .map(({ resource }) => resource), [settings.resources])
  ...
  const track = (
    <div className="relative flex">
      ...
      <EventCalendarTimeGutter days={[day]} slots={slots} startHour={startHour} interval={interval} />
        ...
        {showAllDay && (
          <div
            ...
            {viewConfig.renderAllDaySection?.({
              ...
              <>
                <div
                  className={cn(// pt-1.5 matches the all-day cell's top inset; the inner box is
                ...
                <div className="grid min-w-0 flex-1" style={{ gridTemplateColumns }}>
                  {resources.map((resource) => (
                    <EventCalendarResourceAllDayCell key={resource.id} resource={resource} day={day} />
                  ))}
                </div>
              </>
            )}
...
function EventCalendarResourceAllDayCell({
  ...
  const mine = segments.allDay.filter((segment) => segment.occurrence.event.resourceId === resource.id)
  ...
  return (
    <div
      ...
      className={cn(// reserve one bar row so the all-day row keeps the same height with or
      // without events, matching the time-grid all-day row (which reserves
      // the same via its bars-grid minHeight)
      "relative flex min-h-[calc(var(--ec-month-bar-h,1.625rem)+0.625rem)] min-w-0 flex-col gap-0.5 border-e px-1 py-1.5 last:border-e-0", isOff && offClassName, viewConfig.dayClassName?.(day), inDraft &&
        cn(
