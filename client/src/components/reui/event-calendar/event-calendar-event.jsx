...
function EventCalendarEvent(
  ...
  const resizeOn = useEventCalendarSelector((state) => state.interactions.resize, { calendar: instance })
  ...
  const defaultContent = (
    <>
      ...
      <span
        className={cn("font-medium", stackedBlock ? EVENT_CALENDAR_FADE_TRUNCATE : "truncate")}>
        {event.title}
      </span>
  ...
  const agendaTimeText = (() => {
    ...
    if (endsAfter) {
      return settings.i18n.labels.timeFrom(format(
        toZoned(occurrence.start, settings.timeZone),
        settings.i18n.formats.eventTime,
        { locale: settings.locale }
      ));
    ...
    if (startsBefore) {
      return settings.i18n.labels.timeUntil(format(
        toZoned(occurrence.end, settings.timeZone),
        settings.i18n.formats.eventTime,
        { locale: settings.locale }
      ));
  ...
  const defaultProps = {
    ...
    "aria-label":
      settings.i18n.functions.formatEventAriaLabel?.(event.title, timeLabel, segment.continuesBefore || segment.continuesAfter) ??
      `${event.title}, ${timeLabel}${
    ...
    className: cn(
      ...
      view === "agenda"
        ...
        : cn(// @container removes intrinsic sizing; only grid chips are containers
      // py-1: room above/below inline badges (attendee pill etc.)
      "@container gap-1.5 rounded-sm px-1.5 py-1 leading-normal", // soft tint + inset ring, not an accent border: legible on both themes
      "bg-(--ec-event-color)/15 hover:bg-(--ec-event-color)/25", // a flat tint reads darker on a dark surface, so lift it there
      "dark:bg-(--ec-event-color)/20 dark:hover:bg-(--ec-event-color)/30", "inset-ring inset-ring-(--ec-event-color)/15", "transition-[background-color,box-shadow] duration-150", "data-dragging:opacity-40", "data-selected:bg-(--ec-event-color)/30 data-selected:inset-ring-(--ec-event-color)/40", segment.continuesBefore && "rounded-s-none", segment.continuesAfter && "rounded-e-none"),
