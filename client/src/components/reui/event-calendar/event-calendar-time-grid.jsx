...
function EventCalendarTimeGrid({
  ...
  const visibleRange = useEventCalendarSelector((state) => state.visibleRange, {
  ...
  const days = useMemo(() => {
    ...
    const filtered = result.filter((day) =>
  ...
  useEffect(() => {
    ...
    const slotRow = el.querySelector("[data-slot=event-calendar-time-gutter] > div")
    const slotPx = slotRow?.getBoundingClientRect().height || 64
    const pxPerMinute = slotPx / interval
    const scrollTo = (minutes) => {
  ...
  const allDaySegments = useEventCalendarSelector(() => {
    ...
    const result = days.flatMap((day) => byDay.get(getDayKey(day, settings.timeZone))?.allDay ?? [])
  ...
  const track = (
    <div className="relative flex">
      <EventCalendarTimeGutter days={days} slots={slots} startHour={startHour} interval={interval} />
  ...
  const defaultProps = {
    ...
    children: (
      <>
        ...
        <div
          className={cn("flex border-b pe-(--ec-scrollbar-w,0px)", !contained &&
            "bg-background sticky top-(--ec-sticky-offset,0px) z-20", viewConfig.classNames?.timeGridHeader)}>
        ...
        {showAllDay && (
          <div
            ...
            {viewConfig.renderAllDaySection?.({
              ...
              <>
                <div
                  className={cn(// pt-1.5 matches the bar overlay's top inset; the inner box
                  ...
                  "text-muted-foreground w-(--ec-gutter-width,4.5rem) shrink-0 border-e ps-2 pe-2.5 pt-1.5", viewConfig.classNames?.allDayLabel)}>
                  <span
                    className="flex h-[calc(var(--ec-month-bar-h,1.625rem)-0.125rem)] items-center justify-end">
 ...
 */
function useEventCalendarAllDayBars(days) {
  const instance = useEventCalendar()
  const settings = useEventCalendarSettings()
  return useEventCalendarSelector(() => {
    ...
    const packed = Array.from(merged.values()).sort((a, b) =>
...
function EventCalendarAllDayBars({
  ...
  const dragGhost = useEventCalendarSelector((state) => {
...
function EventCalendarAllDayCell({
  day
}) {
  const settings = useEventCalendarSettings()
  const viewConfig = useEventCalendarViewConfig()
  ...
  const isOff = resolveOffDay(day, settings.timeZone, effective.offDays
    ? typeof viewConfig.offDays === "object"
      ? viewConfig.offDays
      : true
    : false, settings.weekendDays)
  const offClassName =
    (typeof viewConfig.offDays === "object" && viewConfig.offDays.className) ||
    "bg-muted/25"

  const isDropTarget = useEventCalendarSelector((state) => {
    const drag = state.drag
    if (!drag || !drag.proposedDayGranular) return null
  ...
  const allDayDraftRange = useEventCalendarSelector((state) => {
    const draft = state.slotDraft
    if (!draft || !draft.allDay) return null
...
function minuteBlockStyle(startMin, endMin, boundsStartMin) {
...
function EventCalendarDayColumn({
  ...
  const draftWindow = useEventCalendarSelector((state) => {
    const draft = state.slotDraft
    if (!draft || draft.allDay) return null
  ...
  const draftRange = useEventCalendarSelector((state) => {
    const draft = state.slotDraft
    if (!draft || draft.allDay) return null
    return { start: draft.start, end: draft.end }
  }, {
  ...
  const packedTimed = useMemo(() => {
    ...
    const clones = visible.map((segment) => ({
      ...segment
    }))
  ...
  const slotFromPointer = e => {
    ...
    const clamped = Math.min(Math.max(minutes, boundsStartMin), boundsEndMin - settings.slotDuration)
    return {
        ...
        <div
          ...
          {dragGhost.kind !== "move" && (
            <EventCalendarEvent
              ...
              className={cn(dragGhost.window[1] - dragGhost.window[0] <
                viewConfig.compactEventMinutes
                ? "h-full gap-1 py-0 leading-4"
                : "h-full flex-col items-start justify-start gap-0 py-1", viewConfig.classNames?.timedChip, "inset-ring-0", !dragGhost.valid && EVENT_CALENDAR_GHOST.invalidContent)} />
          )}
        </div>
      )}
      {/* Drag-create draft */}
...
function EventCalendarNowIndicator(
  ...
  return (
    <div
      ...
      <div
        className="bg-destructive/40 absolute start-(--ec-gutter-width,4.5rem) end-0 h-px" />
      ...
      <div
        ...
        <div
          className="bg-destructive absolute -start-1 top-0 size-1.5 -translate-y-1/2 rounded-full" />
      </div>
    </div>
  );
}

function EventCalendarWeekView(props) {
  return <EventCalendarTimeGrid view="week" {...props} />;
}

function EventCalendarDayView(props) {
  return <EventCalendarTimeGrid view="day" {...props} />;
}

function EventCalendarDaysView(props) {
