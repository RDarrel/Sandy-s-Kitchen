...
import { addDays, addMinutes, setHours, startOfDay, startOfWeek } from "date-fns";
  ...
  if (event.id === "client-call") {
    return (
      <span className="flex w-full min-w-0 items-center gap-1.5">
        ...
        <span className="truncate font-medium">{event.title}</span>
        <span
          className="ms-auto shrink-0 rounded bg-(--ec-event-color)/25 px-1 text-[10px] font-semibold">
          30m
        </span>
      </span>
    );
                      ...
                      <TabsContent value="view" className="flex flex-col gap-3">
                        ...
                        <SettingsSelect
                          ...
                          onValueChange={(weekStartsOn) =>
                            patch({ weekStartsOn: weekStartsOn })
                          } />
                      </TabsContent>
