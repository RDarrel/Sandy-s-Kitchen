...
const flattenArray = (array) => {
  ...
  for (const key in array[0]) {
    const settings = new Map([
      ...
      ["dob", { text: "Birthdate\n(MM/DD/YYYY)" }],
      ["learning", { text: "Learning Modality" }],
...
const set = {
  ...
  banner: ({ worksheet, options }) => {
    ...
    worksheet.mergeCells("I1:Y1");
    const title = worksheet.getCell("I1");
    title.value = file;
    title.font = { bold: true, size: 22, name: "SansSerif" };
    title.alignment = { horizontal: "center" };

    worksheet.mergeCells("N2:U2");
    const description = worksheet.getCell("N2");
  ...
  main: ({ worksheet, head = [], female = [], male = [] }) => {
    ...
    function processArray(array, startPos) {
      for (let i = 0; i < array.length; i++) {
        ...
        const indexOfWasDrop = element.length - 1;
        const wasDrop = element[indexOfWasDrop];
        copyOfElement.splice(indexOfWasDrop, 1);
  ...
  footer: ({ worksheet, skip, male: maleCount, female: femaleCount }) => {
    ...
    worksheet.mergeCells(`A${skip + 6}:G${skip + 6}`);
...
const handleCompress = ({ worksheet }) => {
  ...
  worksheet.getColumn("N").width = 3;
  worksheet.getColumn("O").width = 6;
  worksheet.getColumn("R").width = 6;
  worksheet.getColumn("T").width = 6;
  worksheet.getColumn("Z").width = 5;
  worksheet.getColumn("AB").width = 5;
};
const SF1 = async ({ array = [], options = {} }) => {
