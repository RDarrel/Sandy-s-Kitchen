...
export const INCOMING_ORDERS = createAsyncThunk(
  `${url}/incoming-orders`,
  ({ params }, thunkAPI) => {
    try {
      return axioKit.universal(`${url}/incoming-orders`, params);
    } catch (error) {
      const message =
        (error.response &&
          error.response.data &&
...
export const RECEIVE_DELIVERY = createAsyncThunk(
  ...
  (form, thunkAPI) => {
    try {
      return axioKit.update(url, form.data, "receive-delivery");
    } catch (error) {
      const message =
        (error.response &&
          error.response.data &&
...
export const reduxSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(INCOMING_ORDERS.pending, (state) => {
        state.isLoading = true;
        state.isSuccess = false;
        state.message = "";
      })
      .addCase(INCOMING_ORDERS.fulfilled, (state, action) => {
        const { payload } = action.payload;
        state.collections = state.filtered = payload;
        state.isLoading = false;
      })
      .addCase(INCOMING_ORDERS.rejected, (state, action) => {
        const { error } = action;
        state.message = error.message;
        state.isLoading = false;
      })
